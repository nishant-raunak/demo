...
with open('/Users/nishant/Downloads/mea_ml_train.csv', 'r') as f:
    reader = csv.reader(f)
    mea_fml = list(reader)
    ...
    for each in comb_list:
        ...
        for i, each in enumerate(final_arr_trans_clean):
            ...
            for element in final_arr_trans_clean[i + 1:, ...]:
                if (list(each)[0:-1] == list(element)[0:-1]) and (
                        abs(float(list(each)[-1]) - float(list(element)[-1])) < 1):
                    if list(element) not in measurements_match_list:
                        measurements_match_list.append(list(element))
                    counter += 1
                elif (list(each)[0:-1] == list(element)[0:-1]) and (
                        abs(float(list(each)[-1]) - float(list(element)[-1])) > 1):
                    counter_not += 1
                else:
                    pass
